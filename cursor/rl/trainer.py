...
class CursorRayPPOTrainer:
    ...
    def _dump_generations(self, message_trajectories, gts, scores, reward_extra_infos_dict, dump_path):
        """Dump rollout/validation samples as JSONL."""
        os.makedirs(dump_path, exist_ok=True)
        filename = os.path.join(dump_path, f"{self.global_steps}.jsonl")

        n = len(message_trajectories)
        base_data = {
            # "input": inputs,
            # "output": outputs,
            "message_trajectories": message_trajectories,
            "gts": gts,
            "score": scores,
        ...
        lines = []
        for i in range(n):
            entry = {k: v[i].item() if isinstance(v[i], (np.float64, np.int64)) else v[i] for k, v in base_data.items()}
    ...
    def _maybe_log_val_generations(self, message_trajectories, scores):
        ...
        # samples = list(zip(message_trajectories, scores, strict=True))
        ...
        selected_samples = message_trajectories[:generations_to_log]
        samples: str = ""
        for i, (msg_traj, score) in enumerate(zip(selected_samples, scores)):
            samples += "-" * 64 + "\n"
            samples += f"Sample {i}:\n"
            samples += json.dumps(msg_traj, ensure_ascii=False, indent=2) + "\n"
            samples += f"Score: {score}\n\n"
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        # For agent loop, we need reward model keys to compute score.
        if self.async_rollout_mode:
            # print("get gen batch async_rollout_mode")
            gen_batch.non_tensor_batch.update(batch.non_tensor_batch)
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # sample_item_idx.extend(test_batch.batch["item_idx"].tolist())

            # test_states: List[State] = test_batch.non_tensor_batch["state"]
            # ground_truths = [state.environment.bbox_coords for state in test_states]
            # sample_gts.extend(ground_truths)
            sample_gts.extend([None] * len(test_batch.non_tensor_batch["uid"]))
            ...
            # imgs_to_close = test_gen_batch.non_tensor_batch.pop("image_observation")
            # for img in imgs_to_close:
            #     img.close()
            # for state in test_gen_batch.non_tensor_batch["state"]:
            #     state.environment.screenshot.close()

            if val_data_dir:
                sample_message_trajectories.extend(test_output_gen_batch.non_tensor_batch["message_trajectory"])
            ...
            print(f"completed {len(sample_uids)} samples")

        self.async_rollout_manager.outside_sleep()
        print("validation generation end")

        self._maybe_log_val_generations(message_trajectories=sample_message_trajectories, scores=sample_scores)
        # results = {k: v for k, v in zip(uid, msg_traj) for uid, msg_traj in zip(sample_uids, sample_message_trajectories)}
        # dump generations

        if val_data_dir:
            self._dump_generations(
                message_trajectories=sample_message_trajectories,
                gts=sample_gts,
        ...
        del sample_message_trajectories
        del sample_gts
        del sample_scores
        del sample_turns
        del sample_uids
        del data_sources
        del data_src2var2metric2val
        del test_batch
        del test_gen_batch
        del test_gen_batch_padded
        del test_output_gen_batch_padded
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            ...
            from cursor.rl.cursor_interaction import CursorAgentLoopManager
    ...
    def get_training_batch(self, online_filtering: bool = True, max_times_to_make_a_batch=8) -> DataProto:
        ...
        while try_to_make_a_batch_times < max_times_to_make_a_batch:
            ...
            if online_filtering:
                print(f"trial {try_to_make_a_batch_times}/{max_times_to_make_a_batch} to make a batch")
            try:
    ...
    def fit(self):
        ...
        while self.global_steps <= self.total_training_steps:
            ...
            with marked_timer("step", timing_raw):
                ...
                with marked_timer("get_batch_rollout_with_filtering", timing_raw):
                    batch, gen_batch, filtering_info = self.get_training_batch(
                        online_filtering=self.config.online_filtering,
                        max_times_to_make_a_batch=self.config.max_times_to_make_a_batch,
                    )
                metrics.update(filtering_info)
                metrics.update(
                    {
                        "timing_s/get_batch_rollout_with_filtering": timing_raw.get(
                            "get_batch_rollout_with_filtering", 0.0
                        )
                    }
                )
                # try:
                ...
                # # we do not use repeat for states because it needs deepcopy
                ...
                #         timing_raw.update(gen_batch_output.meta_info["timing"])
                #         gen_batch_output.meta_info.pop("timing", None)
                # gen_batch_output has n * batch_size items
                ...
                # batch.non_tensor_batch.pop("state")
                # batch.non_tensor_batch.pop("raw_messages")
                # imgs_to_close = batch.non_tensor_batch.pop("image_observation")
                # for img in imgs_to_close:
                #     img.close()
            ...
            for key in batch.meta_info["reward_extra_keys"]:
                reward_extra_info[f"reward_extra_info/{key}"] = batch.non_tensor_batch[key].mean()
            metrics.update(reward_extra_info)
